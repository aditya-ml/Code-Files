...
path =r'C:\ADITYA\Computervision\UCF50_videos\UCF50\BaseballPitch'

i=0
prev_i=0
path1 =r'C:\ADITYA\Computervision\UCF50_videos\UCF50\BaseballPitch\\'
for dirpath,dirnames,filename in walk(path):
    while i <=1 :#<= len(filename):
        #prev_i=i;
        count=0
        vidcap=cv2.VideoCapture(os.path.join(path1, filename[i]))
        success,image = vidcap.read()
        while success:
            cv2.imwrite(r"C:\ADITYA\Computervision\UCF50_videos\BaseballPitch_frames\frame%d.jpg" % count, image)     # save frame as JPEG file      
            success,image = vidcap.read()
...
# loop over the image paths
   ...
   # print(a)
    ...
    #   np.savetxt(f_handle,rects)
            ...
            Phase_spectrum=A[1];
            print(i)
            with open('vid_fft%d.csv'%i,'a',newline='') as out:
